...
def wbm_sobol(
    ...
    # Read data
    if forcing == "SMAP":
        forcing_data = np.load(f"{project_data_path}/WBM/calibration/CONUS/{forcing}/inputs.npz")
    else:
        forcing_data = np.load(f"{project_data_path}/WBM/calibration/CONUS/VIC/inputs.npz")  # identical for SA purposes
    ...
    ###################
    if eval == "SMAP":
        obs = np.load(f"{project_data_path}/WBM/calibration/CONUS/{forcing}/{forcing}_validation.npy")
        obs = obs[ix, iy, :]
        obs_centered = obs - jnp.mean(obs)
    elif eval == "NLDAS":
        obs_list = ["VIC", "NOAH", "MOSAIC"]
        obs = [np.load(f"{project_data_path}/WBM/calibration/CONUS/{obs}/{obs}_validation.npy") for obs in obs_list]
    ...
    for iparam in range(n_params):
        ...
        if eval == "SMAP":
            ...
            sims_ubrmse[iparam] = jnp.sqrt(jnp.mean((sim_centered - obs_centered) ** 2))
        elif eval == "NLDAS":
            for io in range(len(obs_list)):
                sims_rmse[io][iparam] = jnp.sqrt(jnp.mean((sim - obs[io]) ** 2))
                sims_ubrmse[io][iparam] = jnp.sqrt(jnp.mean((sim_centered - obs_centered[io]) ** 2))
    ...
    if store_sims: 
        ...
        np.savez(f"{project_data_path}/WBM/SA/{save_name}_sim_stats.npz", **out)
    ...
    df_out_total.to_csv(f"{project_data_path}/WBM/SA/{save_name}_res_total.csv", index=False)
    df_out_second.to_csv(f"{project_data_path}/WBM/SA/{save_name}_res_2order.csv", index=False)
